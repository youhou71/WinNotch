...
- **GitLab — sections pliables dans le panel, avec mémoire.** Un clic sur le bandeau d'une section (« issues à prendre », « mes issues », « à reviewer », « mes MR ») la replie, et l'état est retenu : il survit à la fermeture du panel comme au redémarrage de l'application. Plier ne coupe **rien** — la section continue d'être interrogée et le compteur de son bandeau reste à jour, contrairement à une section décochée dans les réglages, qui n'est plus requêtée du tout. Les deux réglages sont volontairement distincts : l'un range l'affichage, l'autre économise un appel d'API.
- **GitLab — lien direct par section.** Au survol d'un bandeau, une icône ouvre la page GitLab équivalente, filtrée à l'identique de ce que la section liste : mes MR ouvertes en tant qu'auteur, celles où je suis reviewer, le tableau de bord des work items pour mes issues assignées, les issues sans assigné portant les labels surveillés. Un nouveau champ **Réglages → GitLab → Liens du panel** permet de cadrer les liens MR et issues sur un groupe (ex. `app`) ; laissé vide, ils visent le tableau de bord global, qui est la portée réellement interrogée par l'API. Le champ tolère une URL complète collée depuis le navigateur, elle est réduite au chemin du groupe.
...
- **GitLab — chaque liste suivie s'active à la carte.** Le module imposait ses quatre listes à tout le monde, alors que personne n'a le même usage : certains n'ont pas de labels de criticité à surveiller, d'autres ne reviewent jamais. Réglages → GitLab → « Ce qui est suivi » permet désormais de couper chacune indépendamment. Une section décochée n'est **pas simplement masquée, elle n'est plus interrogée** : un appel d'API en moins par cycle de polling, plus de section dans le panel, plus de compteur dans la card (dont la grille se resserre au nombre restant), et plus de toast la concernant faute de données. Conséquences volontaires : sans « issues à prendre », la chip du notch perd son badge rouge ; sans « mes MR », sa pastille de pipeline cassé. Réactiver une section déclenche un rafraîchissement immédiat plutôt que d'attendre le tick suivant — et surtout **sans rafale de toasts** : les trois détecteurs de nouveautés désarment leur référence quand leur section est coupée, sinon toutes les MR et issues déjà présentes seraient signalées comme nouvelles au retour. Le merge des réglages persistés gagne au passage un second niveau pour `gitlab` : le merge par module étant *shallow*, une clé ajoutée plus tard à `sections` ou à `notify` n'aurait jamais atteint une installation existante.
- **GitLab — mes issues assignées.** Le module suivait les MR et les issues critiques *non assignées*, mais rien de ce qui m'est confié : « ce que j'ai à faire » restait invisible depuis le notch. Une section « Mes issues » complète désormais le panel : tout ce qui est ouvert et assigné à mon compte — issues, incidents, tâches et cas de test, soit les *work items* GitLab — avec l'icône du type, le jalon, l'échéance (rouge une fois dépassée) et la date de dernière activité. L'ordre général suit désormais la logique des tickets d'abord, des MR ensuite : à prendre, mes issues, à reviewer, mes MR — le même dans le panel, dans la card et dans les réglages. La card compacte passe de trois à quatre compteurs ; jusqu'à trois ils restent alignés sur une seule ligne, à quatre ils se replient en 2 × 2 dès que la tuile se resserre, plutôt que d'écraser les libellés. Les **epics** sont hors périmètre : côté API REST, ce ne sont pas des issues (endpoint distinct, édition Premium). Aucun recoupement avec « issues à prendre », qui ne remonte que les issues sans assigné. Coût : un appel REST de plus par cycle de polling, dans le même `Promise.all` que les autres.
- **Tâches modifiables et copiables.** Une tâche ne pouvait jusqu'ici qu'être cochée ou supprimée : la moindre coquille imposait de la supprimer puis de la retaper. Un clic sur le libellé le transforme désormais en champ d'édition (Entrée valide, Échap annule, cliquer ailleurs valide), et un bouton révélé au survol copie l'intitulé dans le presse-papier — pour le reprendre dans un ticket, une MR ou un message sans le ressaisir. Le libellé vidé est refusé plutôt que traité comme une suppression : on ne perd pas une tâche par une validation malencontreuse, la croix reste le seul chemin pour supprimer. `done` et la date de création sont préservés — corriger un intitulé ne crée pas une nouvelle tâche. Détail d'implémentation notable : Échap est intercepté sur `window` en phase de capture, car le gestionnaire global de la vue (`useEscapeKey`, sur `document` en capture) consommerait la touche avant le champ et refermerait la vue au lieu d'annuler l'édition.
