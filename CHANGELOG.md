...
- **GitLab — mes work items assignés.** Le module suivait les MR et les issues critiques *non assignées*, mais rien de ce qui m'est confié : « ce que j'ai à faire » restait invisible depuis le notch. Une section « Mes work items » complète désormais le panel, en pendant de « Mes MR » : tout ce qui est ouvert et assigné à mon compte, issues, incidents, tâches et cas de test confondus, avec l'icône du type, le jalon, l'échéance (rouge une fois dépassée) et la date de dernière activité. La card compacte passe de trois à quatre compteurs et se réorganise en 2 × 2 sur une tuile étroite plutôt que d'écraser les libellés. Les **epics** sont hors périmètre : côté API REST, ce ne sont pas des issues (endpoint distinct, édition Premium). Aucun recoupement avec « issues à prendre », qui ne remonte que les issues sans assigné. Coût : un appel REST de plus par cycle de polling, dans le même `Promise.all` que les autres.
