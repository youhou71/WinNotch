...
### GitLab
Suit les **MR à reviewer**, **mes MR ouvertes**, les **issues critiques non assignées** (labels surveillés, ex. `Severity::Critique`) et **mes work items assignés**. Chip avec badge rouge pulse pour les issues à prendre. Dashboard compact jusqu'à 4 chiffres (voir ci-dessous), panel plein dashboard au clic, clic sur une ligne ouvre dans le navigateur. Toasts pour les nouvelles MR assignées et les nouvelles issues critiques. Une MR **disparaît de « à reviewer » dès que tu as donné ta review** (relue, approuvée ou changements demandés).
...
**Ce qui est suivi se choisit** (Réglages → GitLab) : chacune des quatre listes — MR à reviewer, mes MR, mes work items, issues à prendre — s'active indépendamment. Une section décochée n'est **pas interrogée du tout** : un appel d'API en moins par cycle, plus de section dans le panel, plus de compteur dans la card, et plus de toast la concernant. Décocher « issues à prendre » retire aussi le badge rouge de la chip, et « mes MR » la pastille de pipeline cassé — ces indicateurs n'ont plus de source.

**Mes work items** : la dernière section du panel liste ce qui **t'est assigné** et reste ouvert — issues, incidents, tâches et cas de test confondus, avec l'icône du type, le jalon et l'échéance (qui vire au rouge une fois dépassée). Les epics en sont absentes : côté API REST, ce ne sont pas des issues. Aucun recoupement possible avec « issues à prendre », qui ne remonte que ce qui n'a pas d'assigné.
...
CPU/RAM/uptime sont lus en pur Node natif (`os.cpus()`, `os.totalmem()`, `os.uptime()`) — zéro dépendance. Le débit réseau est calculé via `Get-NetAdapterStatistics` (PowerShell résident, protocole JSON une-ligne comme le module VPN) sur deux snapshots consécutifs, en filtrant automatiquement les interfaces loopback / vEthernet / WSL / Bluetooth PAN / pseudo-interfaces. L'utilisateur peut whitelist explicitement certaines interfaces dans Settings.
...
- **Postes bridés par AppLocker / WDAC (`ConstrainedLanguage`)** : certaines politiques d'entreprise forcent PowerShell en mode restreint pour les scripts situés dans les dossiers inscriptibles par l'utilisateur — dont `%LOCALAPPDATA%`, où WinNotch est installé. Le même script lancé depuis un dépôt sous `C:\` tourne, lui, en `FullLanguage` : ce genre de problème est donc **invisible en développement**. Vérifier avec `powershell -NoProfile -File <script.ps1>` contenant `$ExecutionContext.SessionState.LanguageMode`, **depuis le dossier d'installation**. Conséquences : les modules Système (débit réseau), Confidentialité et VPN fonctionnent (leurs scripts ont été écrits pour ce mode), mais **le mode Peek `Alt` et le masquage automatique en plein écran sont indisponibles** — ils reposent sur du P/Invoke `user32` via `Add-Type`, que ce mode interdit par conception. WinNotch le détecte au démarrage et n'essaie pas de les lancer (un avertissement le signale dans les logs).
